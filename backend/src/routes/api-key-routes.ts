...
const apiKeyRoutes: FastifyPluginAsync<PaymasterRoutesOpts> = async (server, { paymaster }: PaymasterRoutesOpts) => {
  ...
  // GET /apiKeys
  server.get('/', { onRequest: [server.authenticate] }, async function (request, reply) {
    try {
      const apiKeys = await server.apiKeyRepository.findAllByUserId(request.user.id);
      apiKeys.forEach((apiKeyEntity: APIKey) => {
        // apiKeyEntity.privateKey = decode(apiKeyEntity.privateKey, server.config.HMAC_SECRET);
        delete (apiKeyEntity as any).privateKey
      });
  ...
  // POST /apiKeys
  server.post('/', { onRequest: [server.authenticate] }, async function (request, reply) {
      try {
        ...
        const apiKeyInstance = await server.apiKeyRepository.findOneByUserIdAndApiKey(request.user.id, body.apiKey);
        if (apiKeyInstance) {
    ...
    // PUT /apiKeys
    server.put('/', { onRequest: [server.authenticate] }, async function (request, reply) {
      try {
        const body = JSON.parse(request.body as string) as ApiKeyDto;
        if (!body) return reply.code(ReturnCode.FAILURE).send({ error: ErrorMessage.MISSING_PARAMS });
        if (!body.apiKey)
    ...
    // DELETE /apiKeys
    server.delete('/', { onRequest: [server.authenticate] }, async function (request, reply) {
      try {
    ...
    // POST /apiKeys/getSupportedNetworks
    server.post('/getSupportedNetworks', { onRequest: [server.authenticate] }, async (request, reply) => {
      try {
        const body: any = JSON.parse(request.body as string);
        if (!body) return reply.code(ReturnCode.FAILURE).send({ error: ErrorMessage.MISSING_PARAMS });
        if (!body.apiKey) {
        ...
        const apiKeyEntity = await server.apiKeyRepository.findOneByUserIdAndApiKey(request.user.id, body.apiKey);
    ...
    // POST /apiKeys/deployVerifyingPaymaster
    server.post('/deployVerifyingPaymaster', { onRequest: [server.authenticate] }, async (request, reply) => {
      try {
        ...
        const apiKeyEntity = await server.apiKeyRepository.findOneByUserIdAndApiKey(request.user.id, apiKey);
    ...
    // POST /apiKeys/addStake
    server.post('/addStake', { onRequest: [server.authenticate] }, async (request, reply) => {
      try {
        if (!request.body) return reply.code(ReturnCode.FAILURE).send({ error: ErrorMessage.MISSING_PARAMS });
